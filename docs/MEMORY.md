...
| `letiende.co` — `FuncionInyectarHostVisitante` (T-0013/T-0014) | `CloudFront Function` fusionada a `main` (PR #20, host embebido; PR #23, prefijo de `/api/*` y `/sitemap.xml`), asociada como `viewer-request` a los behaviors `/cartelera/*` y `/libros/*`. Copia el `Host` real a `x-le-tiende-host` (ver gotcha de `AllViewerExceptHostHeader`) y quita el prefijo de proxy solo de las subrutas `/api/*` y `/sitemap.xml`, dejando intactas las rutas de página (las necesita el Router de Angular con `baseHref` fijo) |
