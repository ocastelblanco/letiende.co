...
| Ágora — integración con el proxy (T-0013) | Código listo en el repo `agora-letiende`, rama `feature/proxy-letiende-co`, PR #58 (03/09/2026, sin fusionar). No crea ni cambia ningún recurso de AWS: `baseHref /cartelera/`, barra de navegación común (solo estado sin sesión), sitemap y redirección 301 desde `agora.letiende.co` (acotada a `/`/`evento/:slug` cross-domain + el resto mismo dominio con el prefijo agregado, para no romper el acceso directo del staff). Detalle completo en `docs/MEMORY.md`/`docs/TODO.md` del propio repositorio de Ágora |
