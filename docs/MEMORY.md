...
| Ágora — integración con el proxy (T-0013), COMPLETA | `agora-letiende`, rama `feature/proxy-letiende-co` (+ 3 ramas de hallazgos posteriores). **PRs #58/#59/#60/#61/#62/#63, todos fusionados**, en producción real (04/09/2026): `baseHref /cartelera/`, barra de navegación común (solo estado sin sesión), sitemap con prefijo, redirección 301 desde `agora.letiende.co` a `agora.letiende.co/cartelera` (mismo dominio, no cross-domain, hasta que exista T-15 — PR #63, incidente real de producción). Verificado en producción con `curl` real y en staging con navegador real |
| Babel — integración con el proxy (T-0014), COMPLETA | `babel-letiende`, rama `feature/proxy-letiende-co` (+ 1 rama de hallazgo posterior). **PRs #111/#112, ambos fusionados**, en producción real (04/09/2026) — mismo patrón que T-0013 (`baseHref /libros/`, barra común, sitemap, redirección 301 mismo dominio hasta T-15). Verificado en producción con `curl` real y en staging con navegador real: 4 llamadas de API (`libros`, `espacios`, `muebles`, `ubicaciones`) responden 200 con el prefijo correcto |
| `letiende.co` — `FuncionInyectarHostVisitante` (T-0013/T-0014), COMPLETA | `CloudFront Function` asociada como `viewer-request` a los behaviors `/cartelera/*` y `/libros/*`. Copia el `Host` real a `x-le-tiende-host` (ver gotcha de `AllViewerExceptHostHeader`) y quita el prefijo de proxy solo de las subrutas `/api/*` y `/sitemap.xml`, dejando intactas las rutas de página (las necesita el Router de Angular con `baseHref` fijo) — **PRs #20 y #23, ambos fusionados a `main` y en producción** (04/09/2026), verificados de punta a punta contra la distribución de producción real (`ER22S2WADMM83`, por su propio dominio de CloudFront, sin alias todavía) |
...
Hallazgos reales de T-0013/T-0014, tras el primer despliegue a staging — **ninguno anticipado por
`tech-specs.md` §7.2/§7.3**, los cuatro verificados en vivo (curl real y/o navegador real):

| Situación | Solución |
|---|---|
| El `Host` real del visitante nunca llega al SSR de Ágora/Babel | La política `AllViewerExceptHostHeader` (obligatoria para que API Gateway no rechace con 403 — ver la fila de arriba) despoja el header `Host` original antes de reenviar. `EmbebidoService` de cada app dependía de leerlo para decidir si mostrar la barra común. Corregido con `FuncionInyectarHostVisitante` (`CloudFront Function`, `viewer-request`), que copia el `Host` real a un header propio, `x-le-tiende-host`, antes de reenviar — cada app lo lee con `request?.headers.get('x-le-tiende-host')` en vez de `Host` |
| `staging.letiende.co/cartelera/sitemap.xml` y `.../libros/sitemap.xml` no respondían (404/302 según la app) | CloudFront reenvía la ruta completa con el prefijo, sin `OriginPath` (ver la fila de arriba) — pero cada API Gateway solo tenía registrada la ruta sin prefijo, `/sitemap.xml`. Corregido registrando también la ruta con prefijo en cada API Gateway (Ágora agregó además una condición en el handler porque inspecciona `rawPath`; Babel no lo necesitó) |
| **El hallazgo más grave, reportado en vivo por el humano ("nada funciona"):** las llamadas a la API propia (`/api/eventos-publicos`, `/api/libros`, etc.) de Ágora/Babel no llegaban a su app cuando la página estaba embebida bajo el proxy | Son rutas **absolutas** (`http.get('/api/…')`), y el navegador las resuelve contra el ORIGEN de la página (`staging.letiende.co`), **ignorando por completo el `<base href>`** — a diferencia de una ruta relativa, una ruta absoluta nunca hereda el prefijo del `baseHref`. Sin prefijo, CloudFront las enrutaba al comportamiento por defecto (este mismo contenedor, que no tiene esas rutas) en vez de a la app real. El primer intento de arreglo (solo del lado de CloudFront, quitando el prefijo de `/api/*` antes de reenviar) fue necesario pero **insuficiente**: el navegador nunca llegó a enviar la URL con el prefijo. El arreglo completo necesitó una segunda pieza, del lado del cliente: extender el `absoluteUrlInterceptor` que cada app ya tenía (para el caso SSR) para anteponer también el prefijo a las llamadas `/api/*` en el navegador, cuando `EmbebidoService.embebido` es `true`. Verificado con navegador real (pestaña de red): las 5 llamadas de API entre las dos apps responden 200 con el prefijo correcto |
| 🔴 **Incidente real de producción (03/09/2026), reportado en vivo por el humano — quinto hallazgo:** `agora.letiende.co` y `babel.letiende.co` (el único acceso público real hoy, porque T-15 — el cutover — todavía no ha ocurrido) quedaron rotos apenas se fusionó T-0013/T-0014: la redirección de `/`/detalle era CROSS-DOMAIN a `letiende.co/cartelera\|libros/...`, un destino que en producción no existía (`letiende.co` sigue en el sitio estático viejo) — el visitante caía en `/eventos`, el fallback de ese sitio viejo | Ambos repos hermanos corrigieron su `src/server.ts`: mientras T-15 no ocurra, toda ruta de esos dominios redirige mismo dominio con el prefijo, sin excepción (`agora-letiende`#63, `babel-letiende`#112, ambos fusionados). La rama cross-domain queda comentada para restaurarse después de T-15. **Lección para toda tarea futura de este tipo:** una funcionalidad que depende de que OTRO repositorio del ecosistema llegue a cierto estado no debe desplegarse a producción de un repositorio ya "listo" antes de verificar que esa dependencia externa existe de verdad |
| **Verificación previa a T-15 (04/09/2026):** ninguna de las tres distribuciones de CloudFront del dominio (actual, staging, producción nueva) emite los encabezados de seguridad que `CLAUDE.md` §5 exige — verificado con `curl -i` real contra las tres | `serverless.yml` de este repositorio nunca declaró un `ResponseHeadersPolicy` de CloudFront — T-0011 nunca lo incluyó, pese a que la regla ya existía en `CLAUDE.md` cuando se planeó. No es una regresión de T-0013/T-0014, es un requisito documentado desde antes que nunca se implementó. Recomendado resolverlo antes o junto con T-15 — ver `tech-specs.md` §7.2 |
