...
| Ágora — integración con el proxy (T-0013) | `agora-letiende`, rama `feature/proxy-letiende-co`, PRs #58/#59/#60 **fusionados** (03/09/2026): `baseHref /cartelera/`, barra de navegación común (solo estado sin sesión), sitemap y redirección 301 desde `agora.letiende.co`. Tras el despliegue real se encontraron y corrigieron dos hallazgos más, ambos verificados en vivo con navegador real: el sitemap bajo prefijo (PR #61, abierto) y las llamadas a `/api/*` sin prefijo en modo embebido (PR #62, abierto) — ver la fila de "Gotchas conocidos" más abajo y `docs/MEMORY.md`/`docs/TODO.md` del propio repositorio de Ágora |
| Babel — integración con el proxy (T-0014) | `babel-letiende`, rama `feature/proxy-letiende-co`, PR #111 (03/09/2026, sin fusionar, 8 commits) — mismo patrón que T-0013 (`baseHref /libros/`, barra común, sitemap, redirección 301), con los mismos dos hallazgos posteriores ya incorporados desde el primer despliegue a staging. Verificado en vivo con navegador real: 4 llamadas de API (`libros`, `espacios`, `muebles`, `ubicaciones`) responden 200 con el prefijo correcto |
| `letiende.co` — `FuncionInyectarHostVisitante` (T-0013/T-0014) | `CloudFront Function` asociada como `viewer-request` a los behaviors `/cartelera/*` y `/libros/*`. Copia el `Host` real a `x-le-tiende-host` (ver gotcha de `AllViewerExceptHostHeader`) — **PR #20, fusionado a `main` y en producción** — y quita el prefijo de proxy solo de las subrutas `/api/*` y `/sitemap.xml`, dejando intactas las rutas de página (las necesita el Router de Angular con `baseHref` fijo) — **PR #23, abierto en staging, todavía sin fusionar** |
